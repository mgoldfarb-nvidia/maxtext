...
def _run_layer_pipeline_checks():
  ...
  with jax.set_mesh(mesh):
    ...
    assert "keep_original_sequence_order_in_group" not in stablehlo, stablehlo
    ...
    for group_id in (161, 162):
      assert any(
          " all-gather(" in line and f'_scheduling_group_id="{group_id}"' in line
          for line in gradient_hlo.splitlines()
      ), gradient_hlo
